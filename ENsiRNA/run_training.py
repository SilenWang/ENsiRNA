...
# Override RNA-FM loading to use local file BEFORE any other imports
RNA_FM_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'RNA-FM_pretrained.pth')
import fm.pretrained
original_fn = fm.pretrained.rna_fm_t12
def rna_fm_t12_local(model_location=None):
    return original_fn(model_location=RNA_FM_PATH)
